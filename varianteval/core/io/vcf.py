...
def get_breakend_chromosome_side(string: str, position_id: bool) -> int:
    """
    :param string: the ALT field of a BND record of a VCF file;
    :param position_id: TRUE=first position in the VCF record (encoded by the CHROM and POS fields); FALSE=second
    position in the VCF record (encoded by the ALT field).
    :return: value defined in ``constants.BREAKPOINT_SIDE``.
    """
    if position_id:
        if string[0] == "[" or string[0] == "]": return BREAKPOINT_SIDE.LEFT
        else: return BREAKPOINT_SIDE.RIGHT
    else:
        if string.find("[") >= 0: return BREAKPOINT_SIDE.LEFT
        else: return BREAKPOINT_SIDE.RIGHT


def get_confidence_interval(record: VariantRecord, is_first_pos: bool, sigma_multiple: int) -> tuple[int, int]:
    """
    Returns the range of uncertainty around a position ``X`` (implicit from the context).
    
    :param is_first_pos: TRUE=first position in a VCF record; FALSE=last position;
    :param sigma_multiple: if the record does not contain coordinates but just the STD, the procedure assumes an
    interval that extends this multiple of the STD on both sides (2 or 3 captures most of a Gaussian);
    :return: First element: quantity to be added to ``X`` to get the first position of the uncertainty interval
    (typically negative or zero). Second element: quantity to be added to ``X`` to get the last position of the
    uncertainty interval (typically positive or zero). If ``record`` does not contain uncertainty information on one
    side of the interval, the corresponding element of the tuple is set to zero. If ``record``does not contain any
    uncertainty information on ``X``, both elements of the tuple are set to zero and ``X`` is assumed to be certain.
    """
    if is_first_pos:
        if VCF_CIPOS_STR in record.info.keys():
            tmp_tuple = record.info.get(VCF_CIPOS_STR).split(VCF_CI_SEPARATOR)
            return int(tmp_tuple[0]), int(tmp_tuple[1])
            # The sign is already ok
    else:
        if VCF_CIEND_STR in record.info.keys():
            tmp_tuple = record.info.get(VCF_CIEND_STR).split(VCF_CI_SEPARATOR)
            return int(tmp_tuple[0]), int(tmp_tuple[1])
            # The sign is already ok
        if VCF_CILEN_STR in record.info.keys():
            tmp_tuple = record.info.get(VCF_CILEN_STR).split(VCF_CI_SEPARATOR)
            return int(tmp_tuple[0]), int(tmp_tuple[1])
            # The sign is already ok
...
def get_confidence_interval_std(record: VariantRecord, is_first_pos: bool) -> float:
    """
    Returns the STD of the range of uncertainty around a position ``X`` (implicit from the context).
    
    :param is_first_pos: TRUE=first position in a VCF record; FALSE=last position.
    """
    for key in (VCF_STD_START if is_first_pos else VCF_STD_END):
        if key in record.info.keys(): return float(record.info.get(key))
    return 0.0
...
def ct2side(record: VariantRecord) -> tuple[int, int]:
    ...
    if value == VCF_CT_325_STR: return 1, 0
    elif value == VCF_CT_523_STR: return 0, 1
    elif value == VCF_CT_525_STR: return 0, 0
    elif value == VCF_CT_323_STR: return 1, 1
    else:
...
def load_vcf(path: str, record_filter: Callable[[VariantRecord], bool], contig_lengths: dict[str, int], callset: sv.Callset):
    ...
    Remark: only contigs involved in some event are loaded in ``sequences``. The basepairs of a contig are not loaded.
    
    Warning: genotype information is not loaded, and objects are not tagged with a sample ID yet. This if left to the
    future.
    
    Warning: the procedure assumes just one ALT per record. This is done just for simplicity and should be generalized
    in the future.
    
    :param record_filter: a function that decides which VCF records to keep (e.g. only records with PASS annotation, or
    only long SVs);
    :param contig_lengths: the keys are assumed to contain every contig name used in ``path`` (lowercase).
    """
    ...
    for record in vcf_file.fetch():
        ...
        if sv_type in [SV_TYPE.DEL, SV_TYPE.DEL_ME]: 
            ...
            # Warning: a DEL/INV is assumed to be a sequence ``aC'e`` where ``C'`` is the reverse-complement of ``C``
            # and the reference is ``aBCDe``. For now we assume that the length of B,D is unknown, so we cannot create
            # adjacencies.
            pass
...
def load_event_del(record: VariantRecord, contig_lengths: dict[str, int], callset: sv.Callset, tmp_event: sv.Event, tmp_adjacency: sv.Adjacency, tmp_breakpoint: sv.Breakpoint, tmp_sequence: sv.Sequence):
    """
    Converts ``record`` into a deletion event and stores it in dictionary ``callset.events``.
    
    Remark: we assume that a DEL record means that the new adjacency between the end and the beginning of the deleted
    unit is observed by the caller. This should be implemented better by using field SVCLAIM from the VCF 4.4 spec.
    """
    tmp_event.clear()
...
def load_event_ins(record: VariantRecord, contig_lengths: dict[str, int], callset: sv.Callset, tmp_event: sv.Event, tmp_adjacency: sv.Adjacency, tmp_breakpoint: sv.Breakpoint, tmp_sequence: sv.Sequence):
    """
    Converts ``record`` into an insertion event and stores it in dictionary ``callset.events``.
    """
    tmp_event.clear()
    ...
    set_precise_breakpoint(tmp_breakpoint, sequence2, 0, BREAKPOINT_SIDE.LEFT)
    breakpoint2 = retrieve_instance(tmp_breakpoint, callset.breakpoints)
    set_precise_breakpoint(tmp_breakpoint, sequence2, sequence2.length - 1, BREAKPOINT_SIDE.RIGHT)
...
def load_event_dup(record: VariantRecord, contig_lengths: dict[str, int], callset: sv.Callset, tmp_event: sv.Event, tmp_adjacency: sv.Adjacency, tmp_breakpoint: sv.Breakpoint, tmp_sequence: sv.Sequence):
    """
    Converts ``record`` into a tandem duplication event and stores it in dictionary ``callset.events``.
    
    Remark: we assume that a DUP record means that the new adjacency between the end and the beginning of the tandem
    unit is observed by the caller. This should be implemented better by using field SVCLAIM from the VCF 4.4 spec.
    """
    tmp_event.clear()
    tmp_event.event_type = SV_TYPE.INS
...
def load_event_cnv(record: VariantRecord, contig_lengths: dict[str, int], callset: sv.Callset, tmp_event: sv.Event, tmp_interval: sv.Reference_Interval, tmp_breakpoint: sv.Breakpoint, tmp_sequence: sv.Sequence):
    """
    Converts ``record`` into a copy-number variation event and stores it in dictionary ``callset.events``. We assume
    that a CNV record means that the evidence for a change in copy number comes from coverage only, i.e. that no new
    adjacency is observed by the caller. This follows the VCF 4.4 spec. of SVCLAIM.
    
    Warning: the procedure does not set the ``copy_number`` field of the reference interval yet.
    """
    tmp_event.clear()
...
def load_event_inv(record: VariantRecord, contig_lengths: dict[str, int], callset: sv.Callset, tmp_event: sv.Event, tmp_adjacency: sv.Adjacency, tmp_breakpoint: sv.Breakpoint, tmp_sequence: sv.Sequence):
    """
    Converts ``record`` into an inversion event and stores it in dictionary ``callset.events``.
    """
    tmp_event.clear()
...
def load_event_invdup(record: VariantRecord, contig_lengths: dict[str, int], callset: sv.Callset, tmp_event: sv.Event, tmp_adjacency: sv.Adjacency, tmp_breakpoint: sv.Breakpoint, tmp_sequence: sv.Sequence):
    """
    Converts ``record`` into an inverted duplication event and stores it in dictionary ``callset.events``. An INVDUP is
    assumed to be a sequence ``xVV'y``, where ``V'`` is the reverse-complement of ``V`` and the reference is ``xVy``.
    This is not equivalent to Sniffles 1's definition (Supplementary Figure 2.4), in which it seems to be ``xVy'V'z``
    where the reference is ``xVyz``. So we are interpreting Sniffles 1's INVDUP as if it were Sniffles 1's DUP+INV (see
    again Supplementary Figure 2.4).
    
    We assume that an INVDUP record means that the new adjacencies between the end and the end of the tandem unit, and
    between the beginning and the right neighborhood of the tandem unit, are observed by the caller.
    """
    tmp_event.clear()
...
def load_event_bnd(record: VariantRecord, is_tra: bool, contig_lengths: dict[str, int], callset: sv.Callset, tmp_event: sv.Event, tmp_adjacency: sv.Adjacency, tmp_breakpoint: sv.Breakpoint, tmp_sequence: sv.Sequence):
    ...
    Warning: instead, we should collect in the same event all the BND records with the same VCF EVENT tag... This is
    left to the future...
    
    :param is_tra: the breakend record is encoded using the TRA (true) or the BND (false) convention.
    """
    tmp_event.clear()
    tmp_event.event_type = SV_TYPE.INVDUP
    ...
    if is_tra:
        ...
        if tmp_tuple[0] == 1: tmp_breakpoint.side = BREAKPOINT_SIDE.RIGHT
        else: tmp_breakpoint.side = BREAKPOINT_SIDE.LEFT
    else:
    ...
    if is_tra:
        if tmp_tuple[1] == 1: tmp_breakpoint.side = BREAKPOINT_SIDE.RIGHT
        else: tmp_breakpoint.side = BREAKPOINT_SIDE.LEFT
    else:
        alt_field = record.alts[0]
...
def retrieve_instance(query: Any, dictionary: Any) -> Any:
    """
    If ``dictionary`` contains an object that is equivalent to ``query``, the procedure returns that object. Otherwise,
    it creates a shallow copy of ``query`` and adds it to ``dictionary``.
...
def load_breakpoint(record: VariantRecord, is_first_pos: bool, breakpoint: sv.Breakpoint):
    """
    Loads in ``breakpoint`` the information about the first or the second position in ``record``.
    
    Remark: the procedure does not set the following fields of the breakpoint, which are left to the caller to
    complete: ``sequence, side, genotype``.
    """
    ...
    if is_first_pos:
        ...
        sv_type = get_sv_type(record)
        # Remark: VCF's pos is one-based, but it actually refers to the position before the variant.
        ...
        if sv_type in [SV_TYPE.DEL, SV_TYPE.DEL_ME]:
            ...
            # Setting the other breakpoint to precise for now. Later it should be merged with the description of the
            # same breakpoint in the symmetrical VCF record.
            position = get_breakend_chromosome_position(record.alts[0])
            set_precise_breakpoint(breakpoint, None, position, -1)
        elif sv_type == SV_TYPE.TRA:
...
def load_sequence(record: VariantRecord, is_first_sequence: bool, contig_lengths: dict[str, int], sequence: sv.Sequence):
    """
    Loads in ``sequence`` the information about the sequence of the first or second position in ``record`` (the first
    position is the one encoded by columns CHROM,POS).
    
    :param contig_lengths: keys are assumed to contain every contig name used in ``record`` (lowercase).
    """
...
def new_precise_breakpoint(sequence: sv.Sequence, position: int, side: int) -> sv.Breakpoint:
    """
    Builds a breakpoint with no uncertainty
    """
    out = sv.Breakpoint()
    set_precise_breakpoint(out, sequence, position, side)
    return out


def set_precise_breakpoint(breakpoint: sv.Breakpoint, sequence: sv.Sequence, position: int, side: int):
