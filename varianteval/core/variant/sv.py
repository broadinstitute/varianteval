...
class Callset:
    ...
    def relax_breakpoints_with_track(self, track: Track, min_intersection: int):
        """
        ...
        """
        if (self.breakpoints is None) or len(self.breakpoints) == 0 or (track is None) or len(track) == 0: return
        i = 0
        j = 0
        n_breakpoints = len(self.breakpoints)
        n_tracks = len(tracks)
        while i<n_breakpoints and j<n_tracks:
            if self.breakpoints[i].position_last < track[j][1]+min_intersection-1:
                i += 1
                continue
            if self.breakpoints[i].position_first > track[j][2]-min_intersection+1:
                j += 1
                continue
            self.breakpoints[i].position_first = min(self.breakpoints[i].position_first,track[j][1])
            self.breakpoints[i].position_last = max(self.breakpoints[i].position_last,track[j][2])
            self.breakpoints[i].update_position(True)
            i += 1
...
class Breakpoint:
    ...
    def is_similar(self, other: Breakpoint, identity_threshold: int, jaccard_threshold: float) -> bool:
        """
    ...
    def move_right(self, bps: int):
        self.position_avg = min(self.position_avg+bps, sequence.length_max-1)
        self.position_first = min(self.position_first+bps, sequence.length_max-1)
        self.position_last = min(self.position_last+bps, sequence.length_max-1)
        update_position(self, False)
    ...
    def relax_by_radius(self, radius: int):
        ...
        self.position_last = min(max(self.position_avg+radius,self.position_last),sequence.length_max)
...
class Sequence:
    ...
    def get_type(self) -> int:
        ...
        """
        if util.is_chromosome(self.name): return 0
