...
class Sequence:
    ...
    def __hash__(self):
        return hash(self.name) ^ hash(self.is_circular) ^ hash(self.sequence) ^ hash(self.length_min) ^ hash(
            self.length_max) ^ hash(self.length_avg) ^ hash(self.length_std) ^ hash(self.length_probability_function)
    ...
    def get_type(self) -> int:
        ...
        """
        if utils.is_chromosome(self.name):
            return 0
        elif self.name == VCF_INSERTION_STRING_NAME:
            return 2
        return 1
...
class Breakpoint:
    ...
    def __hash__(self):
        return hash(self.sequence) ^ hash(self.side) ^ hash(self.position_first) ^ hash(self.position_last) ^ hash(
            self.position_avg) ^ hash(self.position_std) ^ hash(self.position_probability_function)
    ...
    def __lt__(self, other):
        if self.sequence < other.sequence:
            ...
            return False
        if self.position_first < other.position_first:
            return True
        elif self.position_first > other.position_first:
            return False
        if self.position_last < other.position_last:
            return True
        elif self.position_last > other.position_last:
            return False
    ...
    def __str__(self):
        if self.side == BREAKPOINT_SIDE.LEFT:
            ...
            self.left = "["
            self.right = ">"
        return ("%s %s%s%d..(%d,%f)..%d%s" % (
        self.sequence.name, self.position_probability_function, self.left, self.position_first, self.position_avg,
        self.position_std, self.position_last, self.right)), "Genotype: ", (" ".join([str(i) for i in self.genotype]))
    ...
    def is_similar(self, other, identity_threshold: int, jaccard_threshold: float) -> bool:
        ...
        if self.position_first == self.position_last:
            if other.position_first == other.position_last:
                ...
                jaccard = float(min(other.position_last, self.position_last) - max(other.position_first,
                                                                                   self.position_first) + 1) / (
                                      max(other.position_last, self.position_last) - min(other.position_first,
                                                                                         self.position_first) + 1)
                return jaccard >= jaccard_threshold
        else:
            jaccard = float(
                min(other.position_last, self.position_last) - max(other.position_first, self.position_first) + 1) / (
                                  max(other.position_last, self.position_last) - min(other.position_first,
                                                                                     self.position_first) + 1)
            return jaccard >= jaccard_threshold
    ...
    def update_position(self, update_avg: bool):
        ...
        if self.position_first == self.position_last:
            ...
            if update_avg:
                self.position_avg = self.position_first
            self.position_std = 0
        else:
            if self.position_probability_function in [PROBABILITY_FUNCTION.DELTA, PROBABILITY_FUNCTION.UNIFORM]:
                ...
                if update_avg:
                    self.position_avg = (self.position_first + self.position_last) // 2
...
class Reference_Interval:
    ...
    def __lt__(self, other):
        if self.breakpoint1 < other.breakpoint1:
            return True
        elif self.breakpoint1 > other.breakpoint1:
            return False
        if self.breakpoint2 < other.breakpoint2:
            return True
        elif self.breakpoint2 > other.breakpoint2:
            return False
        return False
...
class Adjacency:
    ...
    def __str__(self):
        return str(self.breakpoint1), " -- ", str(self.breakpoint2), "\n", "Genotype: ", (
            " ".join([str(i) for i in self.genotype]))
    ...
    def __str__(self):
        return ("Type: %d \n" % (self.event_type)), "Adjacencies: \n", (
            '\n'.join([str(i) for i in self.adjacencies])), "Intervals: \n", (
                   '\n'.join([str(i) for i in self.reference_intervals])), "Genotype: ", (
                   " ".join([str(i) for i in self.genotype]))
